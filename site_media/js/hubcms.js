...
///////////////////////Image Upload//////////////////////////////
var Upload = function (id, type, attr, img, trigger, relative_url, options) {
    if (!options) {
        options = {};
    } else {
	options = options;
    ...
    iframe.name = 'upload-form' + attr + type + id;
    iframe.id = 'upload-form' + attr + type + id;
    jq(trigger).mouseover(hover);
    jq(trigger).mouseout(unhover);
    jq(trigger).one('click', load_upload_iframe);
    ...
    var hover = function () {
	jq(img).effect("highlight", {}, 1000);
        jq(this).title = "Click to change photo";
    };
    var unhover = function () {
    };
    var load_upload_iframe = function () {
	jq(this).style.display = "none";
        ...
        var src = relative_url + "uploadImageIframe/" + id + "/" + type + "/" + attr + '?page_name=' + page_name;
        if (options.height && options.width) {
            src += '&height=' + options.height +'&width=' + options.width;
        }
        iframe.src = src;
        var existing = jq('#' + iframe.id);
        if (!existing) {
            jq(img).parent().append(iframe);
        } else {
            iframe.style.display = 'inline';
        }
        jq(iframe).load(cancel_or_submit);
    ...
    var cancel_or_submit = function () {
        unhover(evt);
        jq(iframe).one('load', refresh_iframe);
        //hacking around a very strange issue with firefox
        /*for (var i = 0; i < frames.length; i++) {
            if (frames[i].name === iframe.name) {
                frame = frames[i];
            }
        }*/
        //in IE
        if (!frame) {
            frame = frames[iframe.name];
        }
        jq(frame.document.getElementById('cancel_iframe')).click(cancel_iframe);
        jq(frame.document.getElementById('submit_iframe')).click(submit_iframe);
    };
    var cancel_iframe = function () {
        trigger.style.display = "inline";
	jq(trigger).one('click', load_upload_iframe).mouseover(hover).mouseout(unhover);
        iframe.style.display = 'none';
    };
    var refresh_iframe = function () {
        trigger.style.display = "inline";
        jq(img).one('load', ie_refresh_image);
        source_no += 1;
        img.src = "/display_image/" + source_no + "/" + type + "/" + id + "/" + attr;
        jq(trigger).click(load_upload_iframe).mouseover(hover).mouseout(unhover);
        iframe.style.display = 'none';
    };
    var ie_refresh_image = function () {
        img.src = "/display_image/" + source_no + "/" + type + '/' + id + '/' + attr;
    };
};
